...
def IC_vel(N):
    ...
    '''
    vel = np.sqrt(T_0) * np.random.randn(N, 3) ##f(v)=1/\sqrt(2\pi)*1/\sqrt(kbT/m) exp(-v^2/2(\sqrt(kbT/m))^2) \sigma=\sqrt{kbT/m}=\sqrt{T}
...
if __name__ == "__main__":
    ...
    P = 1 - np.mean(Ps) / (3 * N * T) - 16 * np.pi * rho / (3 * T * L_box**3)  # compressibility factor
